...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    // Enterprise Big PDF Optimizations
    _app_options.AppOptions.set("disableAutoFetch", true);
    _app_options.AppOptions.set("disableStream", false);
    _app_options.AppOptions.set("disableRange", false);
    _app_options.AppOptions.set("maxCanvasPixels", 4194304);
    ...
    if (wmParam) {
      const wmText = decodeURIComponent(wmParam);
      const applyTiledWm = function() {
        let wmDiv = document.getElementById("ompdfWatermarkOverlay");
        if (!wmDiv) {
          wmDiv = document.createElement("div");
          wmDiv.id = "ompdfWatermarkOverlay";
          wmDiv.className = "ompdf-tiled-watermark";
          for (let i = 0; i < 9; i++) {
            const item = document.createElement("div");
            item.className = "ompdf-wm-item";
            item.textContent = wmText;
            wmDiv.appendChild(item);
          }
      ...
      if (document.readyState === "complete" || document.readyState === "interactive") {
        ...
        window.addEventListener("DOMContentLoaded", applyTiledWm);
      }
      setTimeout(applyTiledWm, 1000);
    }

    // Enterprise Dark Mode Switcher & Persistence
    const themeBtn = document.getElementById("themeToggle");
    if (themeBtn) {
      if (localStorage.getItem("ompdf_dark_theme") === "1") {
        document.body.classList.add("dark-theme");
      }
      themeBtn.addEventListener("click", function() {
        document.body.classList.toggle("dark-theme");
        const isDark = document.body.classList.contains("dark-theme") ? "1" : "0";
        localStorage.setItem("ompdf_dark_theme", isDark);
      });
    }
